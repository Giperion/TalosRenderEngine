...
/*
Ñîçäàåò íîâûé áëîê äàííûõ â âèäåîïàìÿòè, è ñîçäàåò óêàçàòåëè â ïàìÿòè ïðèëîæåíèÿ
Äëÿ êàæäîãî âûçîâà AllocateBuffer íóæíî äåëàòü âûçîâ Release Buffer
target - òèï áóôåðà GL_ARRAY_BUFFER, GL_ATOMIC_COUNTER_BUFFER, GL_COPY_READ_BUFFER, GL_COPY_WRITE_BUFFER, GL_DRAW_INDIRECT_BUFFER, GL_DISPATCH_INDIRECT_BUFFER, GL_ELEMENT_ARRAY_BUFFER, GL_PIXEL_PACK_BUFFER, GL_PIXEL_UNPACK_BUFFER, GL_QUERY_BUFFER, GL_SHADER_STORAGE_BUFFER, GL_TEXTURE_BUFFER, GL_TRANSFORM_FEEDBACK_BUFFER or GL_UNIFORM_BUFFER
BufferSize - ðàçìåð áóôåðà
*/
...
/*
Çàâåðøàåò ðåäàêòèðîâàíèå íîâîãî áóôåðà è íàïðàâëÿåò äàííûå â âèäåîïàìÿòü.
...
void OpenGL::FreeBuffer(GLuint BufferID)
{
	...
	{
		//Debug::Log(L"(LOW LEVEL CODE) Íåâîçìîæíî óäàëèòü áóôåð! Âîçìîæíà ïîð÷à êó÷è! Îòìåíà îïåðàöèè...", true);
	}
	else
	{
...
void OpenGL::StartOpenGLOperations(DWORD ThreadID)
{
	...
	//if (p_gl == nullptr)
	//{
	//	//Debug::ErrorLog(L"(LOW LEVEL CODE) Ïîïûòêà âûïîëíèòü OGL îïåðàöèè, ïðè íåèíèöèàëèçèðîâàíîì êîíòåêñòå OGL!", true);
	//	return;
	//}
...
void OpenGL::StopOpenGLOperations(DWORD ThreadID)
{
	...
	//if (p_gl == nullptr)
	//{
	//	Debug::ErrorLog(L"(LOW LEVEL CODE) Ïîïûòêà çàêîí÷èòü OGL îïåðàöèè, ïðè óäàëåíîì êîíòåêñòå OGL!", true);
	//	return;
	//}
